...
从另一方面来讲，私有链中，在一系列已知身份的节点中运行区块链。它可以通过在一组毋须信任对方但有相同目的的的实体中保证安全性，就像在商业中进行基金、信息和商品的交换（都为了追求利益，而不需要信任对方）。由于节点身份已知，私有链可以使用传统的拜占庭容错作为共识算法。  
...
Fabric使用了新的区块链架构，具有弹性，灵活性，可扩展性和保密性。作为一种模块化可扩展的带有准入机制的通用区块链，Fabric支持标准通用编程语言编写的分布式应用。这使得Fabric成为私有链的第一个分布式操作系统。
