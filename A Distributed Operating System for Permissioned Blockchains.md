...
区块链可以被如此定义：它是一个不可变账本，记录交易历史，维护一个毋须信任节点间的分布式网络。每个节点维护一个账本的副本。节点执行共识算法来验证交易，把它们分组进区块，并根据区块信息生成哈希链。这个过程将会给交易排序，这对于保证连续性和一致性是十分必要的。区块链技术已经从比特币的名号中脱颖而出，并被广泛地认为是一种在数字世界中进行信任交流很有前景的技术。  
